...
module.exports.createGDPS = async function (custom_url, password, query) {
...
module.exports.createPcDownload = async function (custom_url, name, version) {
    ...
    try {
        await exec(`rm /home/gdps/downloads/${custom_url}/${name}.zip`)
    } catch {}
    ...
    try {
        fs.mkdirSync(`/home/gdps/downloads/${custom_url}`);
    } catch {}
    ...
    await exec(`cd /home/gdps/GDPS_Creator/Game/PC/Working/${custom_url}/getdl && zip -q -r /home/gdps/downloads/${custom_url}/${name}.zip *`)
...
module.exports.createAndroidDownload = async function (custom_url, name, version) {
    ...
    try {
        await exec(`rm /home/gdps/downloads/${custom_url}/${name}.apk`)
    } catch {}
    const gdps_url = `https://${custom_url}.ps.fhgdps.com${slash_count}`
    const gdps_base64_url = Buffer.from(`https://${custom_url}.ps.fhgdps.com${slash_count}`).toString('base64').replaceAll("=", "")
    const boomlings_url = "http://www.boomlings.com/database"
    ...
    await exec(`cd /home/gdps/GDPS_Creator/Game/Mobile/Decompiler/SignAPK && java -jar signapk.jar certificate.pem key.pk8 ${custom_url}.apk ${custom_url}-signed.apk && mv ${custom_url}-signed.apk /home/gdps/downloads/${custom_url}/${name}.apk`)
...
module.exports.forceDeleteGDPS = async function (custom_url, gdps_id, query) {
    try {
        await query(`DROP USER 'gdps_${custom_url}'@'localhost'`)
    } catch {}
    try {
